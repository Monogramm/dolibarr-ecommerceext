...
class eCommerceRemoteAccessMagento
{
    ...
    // TODO Move to the REST API
    /**
     * Magento client SOAP API v1.
     *
     * @var SoapClient
     * @see https://devdocs.magento.com/guides/v2.1/soap/bk-soap.html Magento SOAP API documentation
     */
    private $client;
    /**
     * Magento client SOAP API v1.
    ...
    public function connect()
    {
        try {
            ...
            $params['version'] = SOAP_1_2;

            $magentoUsername = $this->site->user_name;
            $magentoPassword = $this->site->user_password;
            $opts = array(
                'http'=>array(
                    'header' => 'Authorization: Bearer ' . base64_encode($magentoUsername.':'.$magentoPassword)
                )
            );
            $context = stream_context_create($opts);
            $params['context'] = $context;

            $magentoAddress = $this->site->webservice_address;
            $magentoWsAddress = $magentoAddress .(substr($magentoAddress, -1, 1)!='/'?'/':'') . 'soap/?wsdl_list=1';

            dol_syslog(__METHOD__ . ": Connect to API webservice_address=".$magentoWsAddress." user_name=".$magentoUsername." user_password=".$magentoPassword);

            // TODO Add option to manage mode "non WSDL". location and uri should be set on $params.
            $this->client = new SoapClient($magentoWsAddress, $params);

            dol_syslog(__METHOD__ . ": new SoapClient ok. Now we call SOAP login method");

            xdebug_disable();
            $this->session = $this->client->login($magentoUsername, $magentoPassword);
            xdebug_enable();

            dol_syslog(__METHOD__ . ": connected with session=".$this->session);
        ...
        catch (SoapFault $fault)
        {
            ...
            {
                dol_syslog("Failed to login. Enable option ECOMMERCE_DEBUG to get more information in dolibarr_ecommerceext.log", LOG_WARNING);

                // Add debug
                if (! empty($conf->global->ECOMMERCE_DEBUG))
                {
                    $h=fopen(DOL_DATA_ROOT.'/dolibarr_ecommerceext.log', 'a+');
    ...
    public function convertRemoteObjectIntoDolibarrSociete($remoteObject, $toNb=0)
    {
        ...
        $maxsizeofmulticall = (empty($conf->global->ECOMMERCE_MAXSIZE_MULTICALL)?1000:$conf->global->ECOMMERCE_MAXSIZE_MULTICALL);      // 1000 seems ok for multicall.
        $nbsynchro = 0;
        $nbremote = count($remoteObject);
        if ($nbremote)
        {
    ...
    {
        dol_syslog(__METHOD__ . ": getRemoteCategoryTree");
        try {
        ...
        //var_dump($result);
        dol_syslog(__METHOD__ . ": getRemoteCategoryTree end. Result is a tree of arrays with children in attribute children");
        return $result;
    }
    ...
    {
        dol_syslog(__METHOD__ . ": getRemoteCategoryAtt session=".$this->session);
        try {
            //$result = $this->client->call($this->session, 'auguria_dolibarrapi_catalog_category.tree');
        ...
        dol_syslog(__METHOD__ . ": getRemoteCategoryAtt end");
        return $result;
    }*/
    ...
    {
        dol_syslog(__METHOD__ . ": getRemoteAddressIdForSociete remote customer_id=".$remote_thirdparty_id);
        try {
            //$result = $this->client->call($this->session, 'auguria_dolibarrapi_catalog_category.tree');
        ...
        dol_syslog(__METHOD__ . ": getRemoteAddressIdForSociete end");
        return $result;
    }
    ...
    {
        dol_syslog(__METHOD__ . ": getCategoryData remote category_id=".$category_id);
        try {
            //$result = $this->client->call($this->session, 'auguria_dolibarrapi_catalog_category.tree');
        ...
        dol_syslog(__METHOD__ . ": getCategoryData end");
        return $result;
    }
    ...
    public function getRemoteCommande($remoteCommandeId)
    {
        $commande = array();
        try {
            dol_syslog(__METHOD__ . ": getRemoteCommande begin remote order_id=".$remoteCommandeId);
            ...
            dol_syslog(__METHOD__ . ": getRemoteCommande end");
        } catch (SoapFault $fault) {
            $this->errors[]=$this->site->name.': '.$fault->getMessage().'-'.$fault->getCode();
            dol_syslog($this->client->__getLastRequestHeaders(), LOG_WARNING);
            dol_syslog($this->client->__getLastRequest(), LOG_WARNING);
    ...
    {
        dol_syslog(__METHOD__ . ": updateRemoteProduct session=".$this->session." remote_id=".$remote_id." object->id=".$object->id);
        ...
        dol_syslog(__METHOD__ . ": updateRemoteProduct end");
        return $result;
    }
    ...
    {
        dol_syslog(__METHOD__ . ": updateRemoteStockProduct session=".$this->session." product remote_id=".$remote_id." movement object->id=".$object->id.", new qty=".$object->qty_after);
        ...
        dol_syslog(__METHOD__ . ": updateRemoteStockProduct end");
        return $result;
    }
    ...
    {
        dol_syslog(__METHOD__ . ": updateRemoteSociete session=".$this->session." remote_id=".$remote_id." object->id=".$object->id);
        ...
        dol_syslog(__METHOD__ . ": updateRemoteSociete end");
        return $result;
    }
    ...
    {
        dol_syslog(__METHOD__ . ": updateRemoteSocpeople session=".$this->session." remote_id=".$remote_id." object->id=".$object->id);
        ...
        dol_syslog(__METHOD__ . ": updateRemoteSocpeople end");
        return $result;
    }
    ...
    {
        dol_syslog(__METHOD__ . ": updateRemoteCommande session=".$this->session." remote_id=".$remote_id." object->id=".$object->id);
        ...
        dol_syslog(__METHOD__ . ": updateRemoteCommande end");
        return $result;
    }
    ...
    {
        dol_syslog(__METHOD__ . ": updateRemoteFacture session=".$this->session." remote_id=".$remote_id." object->id=".$object->id);
        ...
        dol_syslog(__METHOD__ . ": updateRemoteFacture end");
        return $result;
    }
    ...
    public function createRemoteLivraison($livraison, $remote_order_id)
    {
        ...
        dol_syslog(__METHOD__ . ": createRemoteLivraison session=" . $this->session . " dolibarr shipment id = " . $livraison->id . ", ref = " . $livraison->ref . ", order remote id = " . $remote_order_id);
        ...
        dol_syslog(__METHOD__ . ": createRemoteLivraison end");
        return $result;
    }
